def calendarMatching(calendar1, dailyBounds1, calendar2, dailyBounds2, meetingDuration):
	...
	return getMatchingAvailbilities(flattenCalendar, meetingDuration)


def updateCalendar(calendar, dailyBounds):
	updatedCalendar = calendar[:]
	updatedCalendar.insert(0, ['0:00', dailyBounds[0]])
	updatedCalendar.insert([dailyBounds[1], '23:59'])
...
def getMatchingAvailbilities(calendar, meetingDuration):
	matchingAvailbilities = []
	for i in range(1, len(calendar)):
		start = calendar[i - 1][1]
		end = calendar[i][0]
		availbilityDuration = end - start
		if availbilityDuration >= meetingDuration:
			matchingAvailbilities.append([start, end])
	return list(map(lambda m: [minutesToTime(m[0]), minutesToTime(m[1])], matchingAvailbilities))


def timeToMinutes(time):
	hours, minutes = list(map(int, time.spilt(':')))
