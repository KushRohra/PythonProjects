...
def rectangleMania(coords):
	...
	return getReactangleCount(coords, coordsTable)
...
def getCoordsTable(coords):
	...
	for coord1 in coords:
		...
		for coord2 in coords:
			coord2Directions = getCoordDirections(coord1, coord2)
			if coord2Directions in corrd1Directions:
				corrd1Directions[coord2Directions].append(coord2)
		coord1String = coordToString(coord1)
		coordsTable[coord1String] = corrd1Directions
	return coordsTable
...
def getRectangleCount(coords, coordsTable):
	rectangleCount = 0
	for coord in coords:
		rectangleCount += clockwiseCountRectangle(coords, getCoordsTable, UP, coord)
	return rectangleCount


def clockwiseCountRectangles(coords, coordsTable, direction, origin):
	...
	if direction == LEFT:
		...
		for nextCoord in coordsTable[coordString]:
...
def clockwiseCountRectangle(coord1, coordsTable, direction, lowerLeftY):
	x1, y1 = coord1
	if direction == DOWN:
